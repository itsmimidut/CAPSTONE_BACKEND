...
export const MAX_TRANSACTION_EXPORT_ROWS = 100000;
...
const bookingQuery = async (filters) => {
  ...
  if (filters.search) {
    ...
    where.push(`(b.booking_reference LIKE ? OR CONCAT_WS(' ', b.first_name, b.last_name) LIKE ? OR COALESCE(c.full_name, CONCAT_WS(' ', c.first_name, c.last_name), '') LIKE ? OR COALESCE(items.description, '') LIKE ?)`);
  ...
  const [rows] = await db.query(`
    SELECT CONCAT('booking:', b.booking_id) source_key, b.created_at transaction_date,
      ...
      COALESCE(NULLIF(TRIM(CONCAT_WS(' ', b.first_name, b.last_name)), ''), NULLIF(c.full_name, ''), NULLIF(TRIM(CONCAT_WS(' ', c.first_name, c.last_name)), ''), NULLIF(b.email, ''), 'Guest') customer_name,
    ...
    LEFT JOIN (SELECT booking_id, SUM(refund_amount) refunded_amount FROM refunds WHERE LOWER(refund_status) IN ('approved','refunded','completed') GROUP BY booking_id) refunds ON refunds.booking_id = b.booking_id
...
const posQuery = async (filters) => {
  ...
  const params = [filters.dateFrom, filters.dateTo];
  const channelExpr = `CASE WHEN pt.type = 'E-Shop' THEN 'eshop' WHEN pt.type = 'Delivery' THEN 'restaurant_online' ELSE 'restaurant_pos' END`;
  ...
  if (filters.search) {
    const term = `%${filters.search}%`;
    where.push(`(COALESCE(pt.receipt_no, '') LIKE ? OR CAST(pt.id AS CHAR) LIKE ? OR COALESCE(c.full_name, CONCAT_WS(' ', c.first_name, c.last_name), pt.pickup_name, pt.recipient_name, '') LIKE ? OR COALESCE(items.description, '') LIKE ?)`);
    params.push(term, term, term, term);
  }
  const [rows] = await db.query(`
    SELECT CONCAT('pos:', pt.id) source_key, COALESCE(pt.transaction_date, pt.created_at) transaction_date,
      ...
      COALESCE(NULLIF(c.full_name,''), NULLIF(TRIM(CONCAT_WS(' ', c.first_name, c.last_name)), ''), NULLIF(pt.pickup_name,''), NULLIF(pt.recipient_name,''), 'Walk-in Customer') customer_name,
      ...
      CASE WHEN UPPER(COALESCE(pt.payment_status, '')) IN ('PAID','REFUNDED','PARTIALLY_REFUNDED','PARTIAL_REFUND') THEN COALESCE(pt.total_amount, 0) ELSE 0 END gross_revenue,
